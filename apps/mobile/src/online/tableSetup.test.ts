...
import { loadBundledPack } from '../pack/offline'
import { acceptedFriends, cleanTableName, freeCharacters, friendshipWith, hostOf, isValidSessionCode, knownUsers, memberLine, PACK_OPTION, pendingReceived, seatLabel, suggestedSessionCode, takenCharacters } from './tableSetup'
...
describe('mesa nueva', () => {
  it('manda el pack empaquetado con el ruleset versionado, como espera la API', () => {
    expect(PACK_OPTION).toEqual({ id: 'pilot', version: '0.4.0', ruleset: 'fantasy-d20-lite@1.0.0' })
  })
...
describe('personajes', () => {
  it('excluye los que otros juegan, conserva el del propio asiento y sabe quien tiene cada uno', async () => {
    const { pack } = await loadBundledPack()
