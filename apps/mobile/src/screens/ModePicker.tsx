...
import {
  ...
  normalizeBaseUrl,
  packArtUrl,
  type CatalogWorldCard,
  type SeasonPassOffer,
  type SeasonPath,
} from "@rpg-ngn/api-client";
...
/** El catalogo publico, para que el Inicio diga lo mismo que la web. Sin red, las tarjetas de siempre. */
function useCatalog(): {
  ...
  useEffect(() => {
    ...
    void storage
      ...
      .then(async (url) => {
        ...
        if (alive)
          setLive({
            worlds: r.worlds.filter((w) => w.origin === "oficial"),
            season: r.season,
            pass: r.pass,
            base: client.baseUrl,
          });
...
const WORLDS: Array<{
  ...
  {
    image: require("../../assets/mundo-valdoria.webp"),
    title: "Fantasía medieval",
    tags: ["Aventura", "Dados"],
  },
  {
    image: require("../../assets/mundo-boticaria.webp"),
    title: "China antigua",
    tags: ["Intriga", "Misterio"],
  },
  {
    image: require("../../assets/mundo-mascarada.webp"),
    title: "Romance",
    tags: ["Drama", "Social"],
  },
];
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.body}>
            Explora, crea y vive historias en cualquier universo: tu novela
            ligera o tu campaña de rol, con amigos o sola, y un director de
            juego que no se cansa. Tú decides el mundo.
          </Text>
                  ...
                  >
                    ...
                    >
                      <View style={styles.worldShade}>
                        <Text style={styles.worldKicker}>
                          {w.catalog.genre}
                        </Text>
                        <Text style={styles.worldTitle}>{w.name}</Text>
                        <Text style={styles.worldState}>
                          {view.badge ?? view.hint ?? view.price ?? ""}
                        </Text>
                      </View>
                    </ImageBackground>
                  </Pressable>
          ...
          {live && destacados.length < FILA ? (
            <View style={[styles.world, styles.proximo]}>
              ...
              <Text style={styles.proximoText}>
                Un mundo nuevo se está escribiendo.
              </Text>
            </View>
          ) : null}
        </ScrollView>
        ...
        {live?.season && live.season.worlds.length > 0 ? (
          ...
          >
            ...
            <Text style={styles.body}>
              Cada turno que juegas es un capítulo. Los capítulos abren mundos
              nuevos, y lo que abres se queda contigo.
            </Text>
            <Text style={styles.seasonPath}>
              {live.season.worlds
                .map(
                  (w) =>
                    `${names[w.packId] ?? w.packId}: ${w.threshold === 0 ? "gratis" : `${w.threshold} capítulos`}`,
                )
                .join("  ·  ")}
            </Text>
            {pase ? (
              <View style={styles.pass}>
                <Text style={styles.passTitle}>
                  Pase de temporada · Capítulos x2
                </Text>
                <Text
                  style={styles.passPrice}
                >{`${pase.price}, pago único`}</Text>
              </View>
            ) : null}
          </Pressable>
        ) : null}
        ...
        >{`Sin conexión puedes leer ${packName}: sus sesiones, fichas y reglas.`}</Text>
      </ScrollView>
