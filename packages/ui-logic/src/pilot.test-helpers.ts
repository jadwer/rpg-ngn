...
import { loadPack, parseEventLog, type CampaignEvent, type FileSource, type LoadedPack, type Session } from '@rpg-ngn/content'
import { fantasyD20Lite } from '@rpg-ngn/rules'

/** Solo para tests: el pack y el log reales del piloto, ya reducidos. */
...
export async function loadPilot(): Promise<Pilot> {
  ...
  return { pack, events: parsed.events, state: reduce(parsed.events, { pack, ruleset: fantasyD20Lite }) }
}

export function sessionOf(pack: LoadedPack, id: string): Session {
  const session = pack.sessions.get(id)
  if (!session) throw new Error(`no hay sesion ${id} en el pack`)
  return session
}
