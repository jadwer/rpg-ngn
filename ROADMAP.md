...
- [ ] **Sin correo saliente (supuesto escrito, caduca con el SMTP).** Mientras `MAIL_MAILER=log`, el correo de la cuenta es un identificador, no una prueba de propiedad: cualquiera puede registrarse con una direccion ajena (`ATOMO_REQUIRE_EMAIL_VERIFICATION=false`) y recuperar contraseña no funciona. La via de rescate es el anfitrion, y a los jugadores se les avisa. Este parrafo se borra el dia que haya SMTP, no se archiva
